...
import { GET_MESSAGE } from "helpers/url_helper";
...
function ChatZalo() {
  ...
  const getMessage = async () => {
    const res = await httpService.get(GET_MESSAGE, {
      params: {
        include: ["lead"],
        where: {
          lead: {
  ...
  const toggleSettings = () => {
    setConfig({
      ...config,
      settings_Menu: !config.settings_Menu,
    });
  };
  ...
  const toggleOther = () => {
    setConfig({
      ...config,
      other_Menu: !config.other_Menu,
    });
  };

  const addMessage = async () => {
    const res = await httpService.post("/parse/functions/sendZaloMsg", {
      zaloId: leadDetail?.zaloId,
      message: config.curMessage,
    });
    ...
    const messInteval = setInterval(() => {
      getMessage();
      scrollToBottom();
    }, 3000);

    clearInterval(messInteval);
  };
                      ...
                      <Row>
                        <Col md="4" xs="9">
                          ...
                          <p className="text-muted mb-0">
                            <i
                              className={
                                "mdi mdi-circle text-success align-middle me-1 mdi mdi-circle align-middle me-1"
                              }
                            />
                            Active
                          </p>
                      ...
                      <div className="chat-conversation p-3">
                        <ul className="list-unstyled mb-0">
                          <PerfectScrollbar
                            ...
                            containerRef={ref => (messageBox.current = ref)}
                          >
                            <li>
