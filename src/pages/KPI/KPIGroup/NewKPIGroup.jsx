...
const NewKPIGroup = () => {
  ...
  const onSubmit = async values => {
    try {
      ...
      history.replace(`/kpi-group/${objectId}`);
    } catch (errorRes) {
  ...
  const getKpiGroup = async () => {
    try {
      if (!id) return;
      const kpiUrl = `/parse/classes/KPIAssignment/${id}?include=["team","kpi"]`;
      ...
      rest.reset({
        ...kpiAssignment,
        ...(kpiAssignment?.team && { target: "team" }),
      ...
      const target = (kpiAssignment?.team && "team") || null;
      setTarget(target);
    } catch (errorRes) {
      if (errorRes?.response?.data?.error) {
        toastrErrorAlert(errorRes.response.data.error);
      ...
      <Container fluid>
        <form onSubmit={handleSubmit(onSubmit)}>
          <ViewableCommon
            ...
            caseTrue={
              <HeaderCreateItem title="Thêm / Sửa Chỉ Tiêu Cho Nhóm">
              ...
              <Col hidden>
                <div style={{ minHeight: 89 }}>
                  <SelectConst
                    ...
                    errors={errors}
                    {...rest}
                    options={[{ text: "Đội nhóm", value: "team" }]}
                    disableEmptyOption
                  />
                </div>
              </Col>
